...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onUploadClick }) => {
  const guideSteps = [
    ...
    },
    {
      ...
      description: 'Use the smart chat ("Alice had the burger") or tap items to make assignments.',
    },
    {
      icon: '📊',
      title: '4. Get Summary',
      description: 'Instantly see a clear breakdown of who owes what and share it with your group.',
    },
  ];
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-4 sm:p-8 bg-surface dark:bg-surface-dark rounded-lg shadow-md border border-border dark:border-border-dark overflow-y-auto">
      <div className="flex-grow flex flex-col items-center justify-center">
        ...
        <p className="max-w-md sm:text-lg md:max-w-xl mb-8 text-text-secondary dark:text-text-secondary-dark">The smartest way to split the bill. Just upload a receipt and let our AI handle the math.</p>
        <button 
          onClick={onUploadClick}
          className="bg-secondary dark:bg-secondary-dark hover:bg-secondary-focus dark:hover:bg-secondary-focus-dark text-on-secondary dark:text-on-secondary-dark font-bold py-3 px-6 sm:px-8 rounded-lg transition-transform transform hover:scale-105 mb-12"
        >
      ...
      <div className="w-full max-w-4xl flex-shrink-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {guideSteps.map((step, index) => (
            <div key={index} className="bg-background dark:bg-background-dark p-4 rounded-lg border border-border dark:border-border-dark h-full text-left">
